...
| Sobel Filter     | - 이미지의 경계(Edge)를 검출하기 위해 이미지의 픽셀 값에 대한 그라디언트를 계산한다.<br>- Edge를 계산하는 필터이므로, 필터의 합이 0이다.                                                                                                                    | 비선형 | Edge 검출, 경계 영역 검출          | 상대적으로 낮음 | - Edge 정보를 정확하게 추출하여 이미지의 특징을 강조<br> - 간단한 연산으로 빠른 처리 가능                                                                 | - 노이즈에 민감하며, 경계선이 뚜렷하지 않은 경우 성능이 저하됨                                                                                                 |
