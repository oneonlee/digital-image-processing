...
Mat makePanorama(Mat img_l, Mat img_r, int thresh_dist, int min_matches)
{
    ...
    // imwrite("result/img_kpts_l.png", img_kpts_l);
    // imwrite("result/img_kpts_r.png", img_kpts_r);
    ...
    Mat img_dsc_obj, img_dsc_scene;
    Extractor->compute(img_gray_l, kpts_obj, img_dsc_obj);
    Extractor->compute(img_gray_r, kpts_scene, img_dsc_scene);

    // < 기술자를 이용한 특징점 매칭 >
    BFMatcher matcher(NORM_L2);
    vector<DMatch> matches;
    matcher.match(img_dsc_obj, img_dsc_scene, matches);
    ...
    // imwrite("result/img_matches.png", img_matches);
    ...
    double dist;
    for (int i = 0; i < img_dsc_obj.rows; i++)
    {
    ...
    // imwrite("result/img_matches_good.png", img_matches_good);
    ...
    // imwrite("result/img_pano_cut.png", img_pano_cut);
...
void exPanorama()
{
    Mat matImg1 = imread("img1/pano_center.jpg", IMREAD_COLOR);
    Mat matImg2 = imread("img1/pano_left.jpg", IMREAD_COLOR);
    Mat matImg3 = imread("img1/pano_right.jpg", IMREAD_COLOR);
    ...
    imwrite("result/ex_panorama_result.png", result);
    waitKey();
    destroyAllWindows();
}
